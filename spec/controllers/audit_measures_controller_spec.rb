...
describe AuditMeasuresController do
  describe '#index' do
    let!(:measure1) { create(:audit_measure, name: 'first measure') }
    let!(:measure2) { create(:audit_measure, name: 'second measure') }
    ...
    it 'returns a list of available measures' do
      allow(AuditMeasure).to receive(:all).and_return([measure1, measure2])
