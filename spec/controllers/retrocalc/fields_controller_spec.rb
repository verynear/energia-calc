...
describe Retrocalc::FieldsController do
  describe '#index' do
    let!(:field1) do
      create(:field, name: 'first field', value_type: 'integer')
    end
    let!(:field2) do
      create(:field, name: 'second field', value_type: 'picker')
    end
    let!(:enumeration1) do
    ...
    it 'returns a list of available fields' do
      expect(Field).to receive(:uniq)
        .and_return(double(order: [field1, field2]))

      get :index, wegowise_id: 3
      expect(JSON.parse(response.body)['fields'])
